...
a cosine shoulder curving back in over the marbles (so bounces stay inside), then the
standard Ø56 female socket to receive the last part's spigot. Terminus: no spigot.
...
def profile(belly, floor, socket_d, couple):
    rb, rf, rs = belly / 2, floor / 2, socket_d / 2
...
def main():
    ...
    prof, top = profile(a.belly, a.floor, a.socket, mc.COUPLE_L)
